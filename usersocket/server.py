...
class PromptServer():
    def __init__(self, loop):
        ...
        if args.tls_keyfile and args.tls_certfile:
            try:
                ...
                self.ssl_context.load_cert_chain(args.tls_certfile, args.tls_keyfile)
                logging.info(f"SSL/TLS enabled with {args.tls_certfile} and {args.tls_keyfile}")
            except Exception as e:
                logging.error(f"Failed to load SSL certificates: {e}")
        else:
            # Prioritize secure keys in ~/.ssh/comfyuikeys/
            home_dir = os.path.expanduser("~")
            secure_cert_path = os.path.join(home_dir, ".ssh", "comfyuikeys", "cert.pem")
            secure_key_path = os.path.join(home_dir, ".ssh", "comfyuikeys", "key.pem")

            usersocket_dir = os.path.dirname(__file__) # Fallback: Certs are in same dir as server.py
            local_cert_path = os.path.join(usersocket_dir, "cert.pem")
            local_key_path = os.path.join(usersocket_dir, "key.pem")
            
            if os.path.exists(secure_cert_path) and os.path.exists(secure_key_path):
                cert_path = secure_cert_path
                key_path = secure_key_path
            else:
            ...
            if os.path.exists(cert_path) and os.path.exists(key_path):
                try:
                    ...
                    logging.info(f"SSL/TLS enabled with {cert_path} and {key_path}")
                except Exception as e:
                    logging.error(f"Failed to load SSL certificates: {e}")
        ...
        async def auth_middleware(request: web.Request, handler):
            # Allow static resources for login page
            if request.path in ["/login", "/login.html", "/favicon.ico", "/UIpaper.webp"] or \
               request.path.startswith("/assets/") or \
               request.path.startswith("/extensions/") or \
               request.path.startswith("/static/") or \
               request.path.startswith("/usersocket/"):
                return await handler(request)
                
            # Check for authentication cookie
            auth_cookie = request.cookies.get("comfy_auth")
            if not auth_cookie:
            ...
            if hasattr(self, 'auth_sessions') and auth_cookie in self.auth_sessions:
                last_activity = self.auth_sessions[auth_cookie]
                if time.time() - last_activity > 15 * 60:
                    del self.auth_sessions[auth_cookie]
                    if "text/html" in request.headers.get("Accept", ""):
                        return web.HTTPFound("/login.html")
                    return web.HTTPUnauthorized()
                
                # Update last activity
                self.auth_sessions[auth_cookie] = time.time()
            elif hasattr(self, 'auth_sessions') and auth_cookie not in self.auth_sessions and auth_cookie == "valid_token":
                # Handle server restart case or untracked valid token (for demo "valid_token")
                # In production, this should validate against persistent storage or re-login
                self.auth_sessions[auth_cookie] = time.time()
    ...
    def add_routes(self):
        ...
        async def serve_uipaper(request):
            return web.FileResponse(os.path.join(os.path.dirname(__file__), 'UIpaper.webp'))

        self.app.add_routes([
            web.static('/', self.web_root),
            web.static('/usersocket', os.path.dirname(__file__)), # Serve usersocket directory (which is now current dir)
            web.static('/extensions/usersocket', os.path.join(os.path.dirname(__file__), 'js')), # Serve JS extensions
            web.get('/UIpaper.webp', serve_uipaper), # Serve UIpaper.webp at root
        ])

        # Login route
        async def login_page(request):
            return web.FileResponse(os.path.join(os.path.dirname(__file__), 'login.html'))
        ...
        async def login_handler(request):
            ...
            # Simple hardcoded check for demo purposes - REPLACE WITH REAL AUTH
            if username == "admin" and password == "password":
                response = web.json_response({"status": "success"})
                token = "valid_token" # In real implementation generate a secure token
                response.set_cookie("comfy_auth", token, max_age=3600*24)
                
                # Update last activity time
                if not hasattr(self, 'auth_sessions'):
                    self.auth_sessions = {}
                self.auth_sessions[token] = time.time()
        ...
        async def logout_handler(request):
            ...
            # Optional: Clear session from memory if needed
            auth_cookie = request.cookies.get("comfy_auth")
            if hasattr(self, 'auth_sessions') and auth_cookie in self.auth_sessions:
                del self.auth_sessions[auth_cookie]
        ...
        # Manually register the JS extension if not picked up by default mechanism
        async def logout_js_handler(request):
             return web.FileResponse(os.path.join(os.path.dirname(__file__), 'js', 'logout.js'))
        ...
        self.app.router.add_get('/extensions/Comfy.Logout/logout.js', logout_js_handler) # Match standard extension path structure
    ...
    async def start(self, address, port, verbose=True, call_on_start=None, ssl_context=None):
        await self.start_multi_address([(address, port)], call_on_start=call_on_start, verbose=verbose, ssl_context=ssl_context)

    async def start_multi_address(self, addresses, call_on_start=None, verbose=True, ssl_context=None):
        runner = web.AppRunner(self.app, access_log=None)
        await runner.setup()
        ssl_ctx = ssl_context
        scheme = "http"
        if ssl_ctx is None and args.tls_keyfile and args.tls_certfile:
